...
function draw_chamber(chamber, tile_map, floor_tile_data) {
	...
	for (var tileX = chamber.left; tileX < chamber.right + 1; ++tileX) {		
		instance_create_layer(tileX * tile_size, chamber.top * tile_size,
			instances_layer, obj_rock_a);
		instance_create_layer(tileX * tile_size, chamber.bottom * tile_size,
			instances_layer, obj_rock_a);
	}
	
	for (var tileY = chamber.top; tileY < chamber.bottom + 1; ++tileY) {
		instance_create_layer(chamber.left * tile_size, tileY * tile_size,
			instances_layer, obj_rock_a);
		instance_create_layer(chamber.right * tile_size, tileY * tile_size,
			instances_layer, obj_rock_a);
	}
}

function generateChamber(anchor_x, anchor_y, direction_x, direction_y) {
	var max_width = 20;
	var min_width = 3;
	var max_height = 20;
	var min_height = 3;
...
function generate_floor() {
	...
	for (var i = 0; i < 10; ++i) {
		var chamber = generateChamber(anchor_x, anchor_y, dir_x, dir_y);
